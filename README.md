# Vero DL

Design deliverables for the Vero project — landing pages, wireframes, and case studies.

## Core Offering Document

[Vero Core Offering](https://docs.google.com/document/d/1V6bYGZ9Pwuc6zcqoBsTEaEgIgR4ejyO0zxmeUOKyLh0/edit?tab=t.0) — Full product and service definition for Vero by Datumlabs.

## Contents

| File | Description |
|------|-------------|
| `html/vero-landing.html` | Main landing page |
| `html/vero-wireframe.html` | Wireframe layout |
| `html/vero-dataflow.html` | Data flow diagram page |
| `html/casestudy-govplus.html` | GovPlus case study |
| `html/casestudy-voltera.html` | Voltera case study |
| `html/casestudy-filed.html` | Filed case study |
| `html/core_offering.html` | Core offering pitch deck |
| `html/hadiqa-upwork-profile.html` | Hadiqa Upwork profile |
| `html/nidal-upwork-profile.html` | Nidal Upwork profile |
